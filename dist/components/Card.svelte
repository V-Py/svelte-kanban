...
function modifyProp(prop) {
  ...
  dispatch("cardPropModify", { prop, col: id_col, card: id, value: document.getElementById(input).value });
}
function saveProp(prop) {
  ...
  dispatch("cardPropSaved", { prop, col: id_col, card: id, value: document.getElementById(input).value });
}
function changeCategory(cat_index) {
...
function handleKeyUp(event, source) {
  if (event.keyCode == 13)
    saveProp(source);
}
onMount(() => {
  document.addEventListener("click", function(e) {
    ...
    if (e.target.classList.contains("category-button"))
      return;
    if (e.target.classList.contains("categories-list"))
      return;
    bool_show_cats_list = false;
  });
});
